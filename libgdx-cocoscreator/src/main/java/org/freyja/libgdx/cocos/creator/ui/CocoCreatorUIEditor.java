...
import org.freyja.libgdx.cocos.creator.ui.model.PolygonCollider;
...
/**
 * CocoCreator ui 解析器.根据CocoCreator的ui编辑器生成的json文件,创建出一个对应Group.
 ...
 */
public class CocoCreatorUIEditor {

	final String tag = CocoCreatorUIEditor.class.getName();
	...
	/** 导出的json结构 */
	protected List<ObjectData> nodeList;

	/** 导出的json结构 */
	protected Map<Integer, ObjectData> nodeIdMap;

	/** 导出的json结构 */
	protected Map<String, ObjectData> nodeMap;
	...
	public ObjectData findNode(String nodeName) {
		return nodeMap.get(nodeName);
	}

	public CocoCreatorUIEditor(FileHandle jsonFile) {
		...
		nodeIdMap = new HashMap<Integer, ObjectData>();
		nodeMap = new HashMap<String, ObjectData>();
		for (int i = 0; i < nodeList.size(); i++) {
			ObjectData node = nodeList.get(i);
			if (node.__id__ == 0) {
				node.__id__ = i;
			}
			nodeIdMap.put(node.__id__, node);
			nodeMap.put(node._name, node);

		}
		// 设置child

		for (ObjectData node : nodeList) {

			if (node.getNode() != null) {

				node.setNode(nodeIdMap.get(node.getNode().__id__));
			}

			if (node._children != null) {
				List temp = new ArrayList();
				for (AANode cNode : node._children) {
					ObjectData data = nodeIdMap.get(cNode.__id__);
					cNode = data;
					temp.add(data);
				}
				node._children = temp;
			}
			if (node._components != null) {
				List temp = new ArrayList();

				Map map = new HashMap();
				for (AANode cNode : node._components) {
					ObjectData data = nodeIdMap.get(cNode.__id__);
					temp.add(data);
					map.put(data.__type__, data);
				}
				node._components = temp;
				node.componentMaps = map;
			}
		}

		AANode n = nodeMap.get("yd_Angel");

		PolygonCollider c = (PolygonCollider) n.componentMaps.get("cc.PolygonCollider");
	 ...
	 */
	public CocoCreatorUIEditor(FileHandle jsonFile, Collection<TextureAtlas> textureAtlas) {
	 ...
	 *            默认ttf字体文件
	 */
	public CocoCreatorUIEditor(FileHandle jsonFile, Map<String, FileHandle> ttfs, Map<String, BitmapFont> bitmapFonts,
	...
	public Group createGroup() {
		...
		//
		Actor actor = parseWidget(null, nodeList.get(1));
	...
	public void debug(ObjectData option, String message) {
		Gdx.app.debug(tag, "控件: " + option.__type__ + "," + option.getName() + " " + message);
	}
	...
	public Actor parseWidget(Group parent, ObjectData widget) {
		...
		String className = widget.__type__;
